...
local available_w = dimen.w - 2 * border_size
local available_h = dimen.h - 2 * border_size

-- Nastavení aspect ratio 1:1.6 (šířka:výška) pro obálky knih
local target_aspect_ratio = 2.0 / 3.0  -- = 0.625

-- Vypočítej max_img_w a max_img_h podle aspect ratio
local max_img_w, max_img_h
if available_w / available_h > target_aspect_ratio then
    -- Omezeno výškou
    max_img_h = available_h
    max_img_w = math.floor(available_h * target_aspect_ratio)
else
    -- Omezeno šířkou
    max_img_w = available_w
    max_img_h = math.floor(available_w / target_aspect_ratio)
end
    local cover_specs = {
...
width = max_img_w,
height = max_img_h,
stretch_limit_percentage = 90, -- G_reader_settings:readSetting("screensaver_stretch_limit_percentage")?
                }
...
function MosaicMenu:_updateItemsBuildUI()
    ...
    for idx = 1, self.perpage do
        ...
        if idx % self.nb_cols == 1 then -- new row
            ...
            table.insert(self.item_group, ptutil.thinWhiteLine(line_width))
